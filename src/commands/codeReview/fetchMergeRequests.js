  ...
  try {
    ...
    if (!gitlabService.isConfigured()) {
      ...
      if (addToken) {
        ...
        if (!isConnected) {
          logger.error('Could not connect to GitLab with the provided token. Please check your configuration.');
    ...
    const detailsSpinner = ora(`Fetching merge request details for MR #${parsedUrl.mergeRequestIid}...`).start();
    
    const mergeRequestDetails = await gitlabService.getMergeRequest(projectId, parsedUrl.mergeRequestIid);
    if (!mergeRequestDetails) {
      detailsSpinner.fail(`Failed to get merge request details for MR #${parsedUrl.mergeRequestIid}`);
      return;
    }
    ...
    const changes = await gitlabService.getMergeRequestChanges(projectId, parsedUrl.mergeRequestIid);
    ...
    const supportedExtensions = config.get('supportedExtensions', ['.php', '.js', '.jsx', '.ts', '.tsx', '.py']);
    ...
    if (supportedChanges.length === 0) {
      changesSpinner.fail(`No supported file types found. Supported extensions: ${supportedExtensions.join(', ')}`);
      ...
      if (customizeExtensions) {
        ...
        if (reFilteredChanges.length === 0) {
          logger.error(`Still no supported files found with extensions: ${supportedExtensions.join(', ')}`);
          return;
        }
      ...
      changesSpinner.succeed(`Found ${supportedChanges.length} files with changes (from total ${changes.length} changed files).`);
    }
...
async function processChanges(changes, projectId, sourceBranch) {
  ...
  for (const change of changes) {
    const changeType = change.new_file ? 'added' : 
    ...
    if (change.diff) {
      ...
      if (changeType === 'added') {
        ...
        if (change.new_path) {
          fullFileContent = await gitlabService.getFileContent(projectId, change.new_path, sourceBranch);
        }
      } 
      ...
      // If it's a modification, we need the complete file content for context
        ...
        try {
          ...
          logger.warn(`Could not process diff for ${change.new_path || change.old_path}: ${error.message}`);
        }
      }
    }
